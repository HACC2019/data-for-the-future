...
export class MyImpact extends React.Component {
  ...
  myProjectsList = this.projectList.filter((item) => (
    item.people.includes(10) || item.hostid == 10
    ))

    projH = this.myProjectsList.filter((item) => (
      item.hostid == 10
    )).length;

    projP = this.myProjectsList.filter((item) => (
      item.hostid != 10
    )).length;

  cat1 = this.myProjectsList.filter((item) => (
    item.categories.includes(1)
    )).length;
  cat2 = this.myProjectsList.filter((item) => (
    item.categories.includes(2)
    )).length;
  cat3 = this.myProjectsList.filter((item) => (
    item.categories.includes(3)
    )).length;
  cat4 = this.myProjectsList.filter((item) => (
    item.categories.includes(4)
    )).length;
  cat5 = this.myProjectsList.filter((item) => (
    item.categories.includes(5)
    )).length;
  cat6 = this.myProjectsList.filter((item) => (
    item.categories.includes(6)
    )).length;

    totalprojects = this.cat1+this.cat2+this.cat3+this.cat4+this.cat5+this.cat6;
  ...
  // check if  date.getMonth()  == 9, 10, 11, 0
  // plot oct, nov, dec, jan

  oct = this.myProjectsList.filter((item) => (
    new Date(item.endsAt).getMonth() == 9
    )).length;
  nov = this.myProjectsList.filter((item) => (
    new Date(item.endsAt).getMonth() == 10
    )).length;
  dec = this.myProjectsList.filter((item) => (
    new Date(item.endsAt).getMonth() == 11
    )).length;
  jan = this.myProjectsList.filter((item) => (
    new Date(item.endsAt).getMonth() == 0
    )).length;
  ...
  barData = {
    ...
    datasets: [
      {
        data: [this.oct,this.nov,this.dec,this.jan]
      }
    ]
  }
  ...
  chartsConfig = {
    ...
    backgroundGradientFrom: '#ebe8f0',
    backgroundGradientTo: '#ebe8f0',
      ...
      <ScrollView style={styles.container} bounces={false} bouncesZoom={false} 
        ...
        <View style={{ margin:10, }}>
          ...
          <View style={{flex: 3, flexDirection: 'column', justifyContent: 'center',alignItems:'center',alignContent:'center'}}>
            ...
            <Text style={styles.levelLabel} category='s1'> 
                 Participating
            </Text>
          </View>
